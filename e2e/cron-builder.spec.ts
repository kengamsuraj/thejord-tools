...
test.describe('Cron Expression Builder E2E', () => {
  ...
  test('should show human-readable description', async ({ page }) => {
    ...
    const description = page.locator('p').filter({ hasText: /minute|hour|day|midnight|00:00/ });
  ...
  test('should display cron format reference table', async ({ page }) => {
    ...
    await expect(page.locator('text=Minute')).toBeVisible();
    await expect(page.locator('text=0-59')).toBeVisible();
    await expect(page.locator('text=Day of Week')).toBeVisible();
  });
